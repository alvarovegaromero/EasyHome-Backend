...
class SharedBoardViewTest(TestCase):
    ...
    def test_post_board_non_existent_group(self):
        response = self.client.post('/api/shared_board/9999', {'content': 'New Content'})
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        self.assertEqual(response.data['error'], "Group wasn't found")

    def test_post_board_not_member(self):
        other_user = User.objects.create_user(username='otheruser', email='otheruser@test.com', password='testpassword')
        other_token = Token.objects.create(user=other_user)
        self.client.credentials(HTTP_AUTHORIZATION='Token ' + other_token.key)
        response = self.client.post(self.url, {'content': 'New Content'})
