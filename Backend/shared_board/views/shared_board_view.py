...
class SharedBoardView(APIView):
    ...
    def put(self, request, group_id):
        try:
            ...
            if last_edited is not None and str(
                    board.last_edited) != last_edited:
                return Response(
                    {'error': 'The board has been edited by another user.'}, status=status.HTTP_409_CONFLICT)
            ...
            return Response({'message': 'Board content updated successfully.',
                             'content': board.content}, status=status.HTTP_200_OK)

        except Exception as e:
            logger.error(
                "An error occurred during shared board edition: %s" %
                str(e))
    ...
    def get(self, request, group_id):
        try:
            ...
            logger.error(
                "An error occurred during shared board retrieval: %s" %
                str(e))
            return Response({'error': "Internal Server Error"},
                            status=status.HTTP_500_INTERNAL_SERVER_ERROR)
