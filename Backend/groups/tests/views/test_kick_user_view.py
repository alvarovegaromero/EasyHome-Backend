...
class GroupKickUserAPIViewTest(TestCase):
    def setUp(self):
        ...
        self.user_to_kick = User.objects.create_user(username='testuser2', email='testuser2@test.com', password='testpassword2')
    ...
    def test_kick_user(self):
        ...
        self.assertEqual(response.data['success'], 'User has been succesfully kicked from the group')
    ...
    def test_kick_user_not_member(self):
        user_not_member = User.objects.create_user(username='testuser3', email='testuser3@test.com', password='testpassword3')
    ...
    def test_kick_user_not_owner(self):
        self.client.credentials(HTTP_AUTHORIZATION='Token ' + Token.objects.create(user=self.user_to_kick).key)
        response = self.client.post(f'/api/groups/{self.group.id}/kick/{self.owner.id}')
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertEqual(response.data['error'], 'Only the group owner can kick users')

    def test_kick_self(self):
        response = self.client.post(f'/api/groups/{self.group.id}/kick/{self.owner.id}')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data['error'], 'You can not kick yourself. Leave the group instead')
