...
class GroupChangeOwnerAPIViewTest(TestCase):
    def setUp(self):
        ...
        self.new_owner = User.objects.create_user(username='testuser2', email='testuser2@test.com', password='testpassword2')
    ...
    def test_change_owner(self):
        ...
        self.assertEqual(response.data['success'], 'Ownership of the group has been changed succesfully')
    ...
    def test_change_owner_not_member(self):
        ...
        response = self.client.post(f'/api/groups/{self.group.id}/change_owner/{user_not_member.id}')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        self.assertEqual(response.data['error'], "User is not a member of this group")

    def test_change_owner_not_owner(self):
        self.client.credentials(HTTP_AUTHORIZATION='Token ' + Token.objects.create(user=self.new_owner).key)
        response = self.client.post(f'/api/groups/{self.group.id}/change_owner/{self.new_owner.id}')
