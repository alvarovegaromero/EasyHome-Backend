    ...
    def post(self, request, group_id):
        try:
            ...
            if group.owner == request.user:
                ...
                # the next user
                next_owner_user_group = UserGroup.objects.filter(
                    group=group).exclude(
                    user=request.user).order_by('join_date').first()
            ...
            return Response(
                {'success': 'You have left the group successfully.'}, status=status.HTTP_200_OK)
