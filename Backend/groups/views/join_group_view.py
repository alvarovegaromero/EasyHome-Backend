    ...
    def post(self, request):
        ...
        try:
            ...
            group = Group.objects.filter(
                join_code=join_code,
                join_code_expiration__gt=timezone.now()).first()

            if group is None:
                return Response(
                    {'error': 'Invalid or expired join code'}, status=status.HTTP_400_BAD_REQUEST)

            if UserGroup.objects.filter(
                    user=request.user, group=group).exists():
                return Response(
                    {'error': 'You are already a member of this group'}, status=status.HTTP_400_BAD_REQUEST)

            UserGroup.objects.create(user=request.user, group=group)

            return Response({'success': 'You have joined the group successfully.',
                             'id': group.id}, status=status.HTTP_200_OK)
